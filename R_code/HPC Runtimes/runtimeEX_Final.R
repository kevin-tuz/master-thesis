...
# Report the parameter
  print(paste0("Starting the simulation for exchangeability at ", Sys.time(), " with following parameters:"))
...
for (i in 1:N) {
  ...
  # Running simulation (Method 1)
    ...
    try({ m1.Reference.unm <- exchTest_genestNeslehovaQuessy(x.unm) })
    try({ m1.Reference.mod <- exchTest_genestNeslehovaQuessy(x.mod) })
    ...
    try({ m2.VReduct.unm <- exchTest_genestNeslehovaQuessy(x.unm.VReduct) })
    try({ m2.VReduct.mod <- exchTest_genestNeslehovaQuessy(x.mod.VReduct) })
    m2.VReduct.benchmark <- difftime(Sys.time(), tm, units = "secs")[[1]]

    # Running simulation (Method 3.1)
    tm <- Sys.time()
    m3.GReduct10.unm <- list(statistic = NA, p.value = NA)
    m3.GReduct10.mod <- list(statistic = NA, p.value = NA)
    m3.GReduct10.benchmark <- difftime(Sys.time(), tm, units = "secs")[[1]]
    ...
    try({
      ...
      m3.GReduct25.unm <- exchTest_genestNeslehovaQuessy(x.unm.GReduct.25)
    })
    try({
      x.mod.GReduct.25 <- greedy_samplereduction_BIV(x.mod, n.greedy.25)
      m3.GReduct25.mod <- exchTest_genestNeslehovaQuessy(x.mod.GReduct.25)
    })
    ...
    try({m4.RndRanks.unm <- exchTest_genestNeslehovaQuessy(x.unm, ties.method = "random")})
    try({m4.RndRanks.mod <- exchTest_genestNeslehovaQuessy(x.mod, ties.method = "random")})
    ...
    try({m5.PiJitter.unm <- exchTest_genestNeslehovaQuessy(x.unm + noise.PiJitter)})
    try({m5.PiJitter.mod <- exchTest_genestNeslehovaQuessy(x.mod + noise.PiJitter)})
    ...
    try({m6.TauJitter.unm <- exchTest_genestNeslehovaQuessy(x.unm + noise.TauJitter.unm)})
    try({m6.TauJitter.mod <- exchTest_genestNeslehovaQuessy(x.mod + noise.TauJitter.mod)})
    ...
    try({m7.Kojadinovic.unm <- copula::exchTest(x.unm, ties = TRUE)})
    try({m7.Kojadinovic.mod <- copula::exchTest(x.mod, ties = TRUE)})
    ...
    try({
      x.unm.VReduct <- fulltie_samplereduction(x.unm)
      m8.VReductRndRanks.unm <- exchTest_genestNeslehovaQuessy(
        x.unm.VReduct, ties.method = "random")
    })
    try({
      x.mod.VReduct <- fulltie_samplereduction(x.mod)
      m8.VReductRndRanks.mod <- exchTest_genestNeslehovaQuessy(
    ...
    try({
      x.unm.GReduct.25 <- greedy_samplereduction_BIV(x.unm, n.greedy.25)
      m9.GReduct25RndRanks.unm <- exchTest_genestNeslehovaQuessy(
        x.unm.GReduct.25, ties.method = "random")
    })
    try({
      x.mod.GReduct.25 <- greedy_samplereduction_BIV(x.mod, n.greedy.25)
      m9.GReduct25RndRanks.mod <- exchTest_genestNeslehovaQuessy(
        x.mod.GReduct.25, ties.method = "random")
    })
    m9.GReduct25RndRanks.benchmark <- difftime(Sys.time(), tm, units = "secs")[[1]]
  
  # Running simulation (Updating data)
    iresult <- list(
    ...
    if (i %% ceiling(N/10) == 0) {
      ...
      write.csv(x <- results, file <- paste0(
          opt$path2, "temp_", round(i / N * 100), "perc_EX_", opt$copula, "_", opt$tau, "_N", N,
...
write.csv(
  x <- results,
  file <- paste0(
      opt$path, "EX_", opt$copula, "_", opt$tau, "_N", N,
      "_n", n,"_rp", rp,"_op", op, "_em", opt$map, ".csv")
)
